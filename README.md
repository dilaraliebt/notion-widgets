# 🌼 Notion Widgets by @dilaraliebt

Welcome & schön, dass du hier bist! 🫶🏼
Auf dieser Seite findest du kleine, frei verwendbare Widgets für Notion, die ich viel Liebe gestaltet habe. Für alle, die ihren digitalen Alltag ein bisschen schöner machen möchten.

---

## 🙋🏻‍♀️ About me

Hi, mein Name ist Dilara, ich bin 32 Jahre alt und bin ein kleiner Notion-Nerd aus Deutschland.
Hauptberuflich arbeite ich als Social Media Managerin & Content Creatorin. In meiner Freizeit blogge ich auf Social Media über mein Leben mit ADHS, grauen Haaren und Mental Health.

---

## 📅 Kalender (`kalender-widget.html`)

Ein ruhiger, cleaner Monatskalender auf Deutsch – perfekt für deine Notion-Seite.

**Features:**
- Woche beginnt mit Montag
- Heutiges Datum wird hervorgehoben
- Farben anpassbar per URL-Parameter
- Navigation zwischen den Monaten

**So bindest du ihn in Notion ein:**
1. Öffne deine Notion-Seite
2. Tippe `/embed`
3. Füge die URL ein und drücke Enter
4. Fertig – dein Kalender erscheint! 🎉

---


```
https://dilaraliebt.github.io/notion-widgets/kalender/kalender-widget.html
```

---

## Catch me on

📸 [Instagram](https://instagram.com/dilaraliebt) 🎥 [TikTok](https://www.tiktok.com/@dilaraliebt) 📺 [YouTube](https://youtube.com/user/dilaraliebt) 📌 [Pinterest](https://de.pinterest.com/dilaraliebt/)

---

Mehr Widgets folgen nach und nach, schau gerne wieder vorbei! 💗

Made with ♥ by Dilara
